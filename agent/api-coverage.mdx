...
The MCP tool reference is exhaustive for the current bridge source in this repository:

```text
Source directory: Parashell-Modules/Source/Bridge/tools
Registration form: @mcp.tool()
Documented tools: 95
Index: /agent/tools
```
...
2. If Python is required, check [Compatible modules](/agent/compatible-modules), [Python API](/agent/python-api), and [TypeIds and properties](/agent/typeids-properties).
...
```text
Class A: Exhaustive source-derived MCP tools.
Class B: Curated FreeCAD-compatible Python API map.
