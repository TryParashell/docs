...
This index lists every tool registered with `@mcp.tool()` in `Parashell-Modules/Source/Bridge/tools`.
...
- `list_documents`: Get the list of currently open documents in Parashell.
  ...
  ```
- `recompute_document`: Recompute one or all open Parashell documents.
