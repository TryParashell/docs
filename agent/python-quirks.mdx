---
title: "Python quirks"
description: "Source-scan Python quirks for Parashell agents using the FreeCAD-compatible API"
---

This page is derived only from scanning `/Parashell`. It records Python-facing behavior that affects agent execution and retrieval.

## Source priority

- Use `.pyi` stubs as the first static callable reference when present. The all-file scan found 277 `.pyi` files, all parse-clean under the active interpreter.
- Treat `.py` implementation files as operational evidence, not as a guaranteed import target. The scan found 34 `.py` files that do not parse under the active interpreter.
- Runtime truth still belongs to the loaded Parashell process: imported modules, `doc.supportedTypes()`, object `PropertiesList`, property metadata, and actual object `TypeId`.
- Do not bulk import arbitrary source files from `/Parashell` for discovery. Use loaded modules or stubs, then confirm with runtime introspection.
...
- Class and attribute docstrings in the stubs are source-adjacent descriptors. Use them as descriptors, but still inspect runtime object properties for dynamic objects.
...
- Spreadsheet formulas in tests use semicolons as function argument separators and support conditional syntax with `? :`.
...
- Tuple/vector/matrix/rotation/placement formula constructors are exercised by the source tests; validate result type and tolerance after recompute.
...
- `/Parashell/src/Mod/Test/TestPythonSyntax.py` demonstrates the intended syntax check: walk Python files, parse with `ast.parse`, and allow explicit whitelist entries.
- The all-file scan found parse failures in generated/templated/workbench files. Treat parse failure as an inspection signal, not as proof the feature is unavailable inside the intended runtime.
- The `.pyi` reference pages are therefore the safer static API surface; the Python source symbol inventory is a discovery index.
